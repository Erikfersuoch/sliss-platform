...
import { fmtDate, daysAgo, addDays, uid, today, isFuReady } from "../helpers.js";
...
const Clients = ({initialClientId,openAdd}) => {
  ...
  const handleAdd=()=>{if(!form.firstName.trim())return;const td=today();const name=form.firstName.trim()+(form.lastName.trim()?' '+form.lastName.trim():'');addRecord("clients",{...form,name,id:uid(),status:"new",firstVisit:td,lastVisit:td,consent:true,created:td});setForm({firstName:"",lastName:"",phone:"",email:"",channel:"WhatsApp",notes:""});setShowNew(false);};
  const handleEdit=()=>{if(!editForm?.firstName?.trim())return;const name=editForm.firstName.trim()+(editForm.lastName.trim()?' '+editForm.lastName.trim():'');const updates={...editForm,name};update("clients",sel.id,updates);setSel({...sel,...updates});setEditMode(false);};
  const handleDelete=id=>{if(window.confirm("Eliminare questo cliente?")){deleteRecord("clients",id);setSel(null);}};
  const openClient=cl=>{setSel(cl);setEditForm({firstName:cl.firstName||cl.name.split(' ')[0]||"",lastName:cl.lastName||cl.name.split(' ').slice(1).join(' ')||"",name:cl.name,phone:cl.phone,email:cl.email,channel:cl.channel,notes:cl.notes||"",status:cl.status});setEditMode(false);setNewApptMode(false);setNewApptDone(false);};
  ...
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      <PageHeader title="Clienti" action={<div style={{display:"flex",gap:"6px",alignItems:"center"}}>{bizType!=="prodotti"&&<><Info {...HELP.invitaCliente} /><Btn v="secondary" s="sm" onClick={()=>setShowInvite(true)}><Icon name="link" size={15} />Invita cliente</Btn></>}<Btn s="sm" onClick={()=>setShowNew(true)}>+ Nuovo</Btn></div>} />
      ...
      {!filtered.length
        ? clients.length===0
          ? <Empty preview={<><GhostRow /><GhostRow /></>} previewLabel="la tua scheda cliente" title="I tuoi clienti vivranno qui" desc="Per ognuno, Sliss tiene la scheda e prepara i follow-up." action={<Btn onClick={()=>setShowNew(true)}>+ Aggiungi il primo cliente</Btn>} hint={bizType!=="prodotti"?<>oppure <button onClick={()=>setShowInvite(true)} style={{background:"none",border:"none",color:T.blue,fontWeight:600,cursor:"pointer",fontFamily:"inherit",fontSize:"12.5px",padding:0}}>invitalo con un link</button></>:undefined} />
          : <Empty icon={<Icon name="search" size={44} color={T.textD} />} title="Nessun risultato" desc="Nessun cliente per questo filtro o ricerca." />
        : <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>{filtered.map((cl,i)=>{const st=CLIENT_ST[cl.status]||{label:cl.status,color:T.textD,bg:T.bg3};return (<Card key={cl.id} hov onClick={()=>openClient(cl)} style={{padding:"13px 16px",animation:`fadeIn .3s ease ${i*.03}s both`}}><div style={{display:"flex",alignItems:"center",gap:"12px"}}><div style={{width:"40px",height:"40px",borderRadius:"50%",background:T.bg3,border:`1px solid ${T.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"16px",fontWeight:700,color:T.textM,flexShrink:0}}>{cl.name.charAt(0).toUpperCase()}</div><div style={{flex:1,minWidth:0}}><div style={{fontWeight:600,fontSize:"15px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{cl.name}</div><div style={{fontSize:"12px",color:T.textD,marginTop:"2px"}}>{cl.channel} · {daysAgo(cl.lastVisit)}</div></div><div onClick={e=>{e.stopPropagation();update("clients",cl.id,{status:nextStatus(cl.status)});}} style={{cursor:"pointer",padding:"4px"}}><Badge {...st} s /></div></div></Card>);})}</div>}
      <Modal open={showNew} onClose={()=>setShowNew(false)} title="Nuovo cliente">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}><FormField label="Nome *"><input value={form.firstName} onChange={e=>setForm(p=>({...p,firstName:e.target.value}))} placeholder="Es. Marco" /></FormField><FormField label="Cognome"><input value={form.lastName} onChange={e=>setForm(p=>({...p,lastName:e.target.value}))} placeholder="Es. Rossi" /></FormField></div>
        ...
        <div style={{display:"flex",gap:"10px",justifyContent:"flex-end"}}><Btn v="secondary" onClick={()=>setShowNew(false)}>Annulla</Btn><Btn onClick={handleAdd} disabled={!form.firstName.trim()}>Salva</Btn></div>
      </Modal>
      <Modal open={!!sel} onClose={()=>{setSel(null);setEditMode(false);setNewApptMode(false);setNewApptDone(false);}} title="Scheda Cliente">
        {sel&&editForm&&(()=>{const st=CLIENT_ST[sel.status]||{label:sel.status,color:T.textD,bg:T.bg3};const fus=(data?.followUps||[]).filter(f=>f.clientId===sel.id).sort((a,b)=>new Date(b.scheduledDate)-new Date(a.scheduledDate));return newApptMode ? (
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"0"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}><FormField label="Nome *"><input value={editForm.firstName} onChange={e=>setEditForm(p=>({...p,firstName:e.target.value}))} /></FormField><FormField label="Cognome"><input value={editForm.lastName} onChange={e=>setEditForm(p=>({...p,lastName:e.target.value}))} /></FormField></div>
            ...
            <div style={{display:"flex",gap:"10px",justifyContent:"flex-end"}}><Btn v="secondary" onClick={()=>setEditMode(false)}>Annulla</Btn><Btn onClick={handleEdit} disabled={!editForm.firstName?.trim()}>Salva</Btn></div>
          </div>
        );})()}
      </Modal>
    </div>
