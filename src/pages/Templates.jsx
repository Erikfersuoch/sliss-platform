  ...
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      ...
      {templates.length===0
        ...
        :<div style={{display:"flex",flexDirection:"column",gap:"10px"}}>{filtered.map((tmpl,i)=>{const ph=PHASES[tmpl.phase]||{icon:"file",label:tmpl.phase||"",color:T.textD,bg:T.bg3};const isEditing=editingId===tmpl.id;return (<Card key={tmpl.id} style={{animation:`fadeIn .3s ease ${i*.04}s both`,opacity:tmpl.active===false?0.6:1}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}><div style={{display:"flex",alignItems:"center",gap:"8px",flex:1,minWidth:0}}><Icon name={ph.icon} size={16} color={ph.color} />{isEditing?<input value={editName} onChange={e=>setEditName(e.target.value)} style={{fontSize:"14px",fontWeight:600,padding:"6px 10px"}} />:<div style={{minWidth:0}}><div style={{fontWeight:600,fontSize:"14px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tmpl.name}</div><div style={{fontSize:"11px",color:T.textD,marginTop:"1px"}}>{tmpl.channel}</div></div>}</div><div style={{display:"flex",gap:"6px",alignItems:"center",flexShrink:0,marginLeft:"8px"}}><Badge {...ph} s />{isEditing?<><Btn v="success" s="sm" aria="Salva modifiche" onClick={()=>saveEdit(tmpl.id)}>{"\u{2713}"}</Btn><Btn v="ghost" s="sm" aria="Annulla" onClick={()=>setEditingId(null)}>{"\u{2715}"}</Btn></>:<><Btn v="ghost" s="sm" aria="Modifica template" onClick={()=>startEdit(tmpl)}><Icon name="pencil" size={15} /></Btn><Btn v="danger" s="sm" aria="Elimina template" onClick={()=>{if(window.confirm("Eliminare?"))deleteRecord("templates",tmpl.id);}}><Icon name="trash" size={15} /></Btn></>}</div></div>{isEditing?<textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{minHeight:"90px",fontSize:"14px",lineHeight:1.7}} />:<div style={{padding:"12px 14px",background:T.bg3,borderRadius:T.r.m,border:`1px solid ${T.border}`,fontSize:"14px",lineHeight:1.7,color:T.textM,whiteSpace:"pre-wrap"}}>{tmpl.text}</div>}{!isEditing&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"10px"}}><button onClick={()=>update("templates",tmpl.id,{active:tmpl.active===false?true:false})} style={{background:"none",border:`1px solid ${tmpl.active===false?T.border:T.green}`,color:tmpl.active===false?T.textMu:T.green,borderRadius:T.r.m,padding:"7px 14px",fontSize:"13px",cursor:"pointer",fontFamily:"inherit",minHeight:"44px"}}>{tmpl.active===false?"○ Disattivato":"● Attivo"}</button><button onClick={()=>navigator.clipboard.writeText(tmpl.text)} style={{display:"inline-flex",alignItems:"center",gap:"5px",background:"none",border:`1px solid ${T.border}`,color:T.textD,borderRadius:T.r.m,padding:"7px 14px",fontSize:"13px",cursor:"pointer",fontFamily:"inherit",minHeight:"44px"}}><Icon name="clipboard" size={14} />Copia</button></div>}</Card>);})}</div>
      }
