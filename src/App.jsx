...
import { PHASES, PRODUCT_PHASES, STATUSES, CLIENT_ST, CLUSTERS_SERVIZI, CLUSTERS_PRODOTTI, CLUSTER_TEMPLATES, MODULES } from "./config.js";
...
const Appointments = () => {
  ...
  const [showSlot,setShowSlot]=useState(false);const [slotDone,setSlotDone]=useState(false);const [slotForm,setSlotForm]=useState({date:today(),serviceType:"",phone:""});const [slotLink,setSlotLink]=useState("");
  ...
  // Link WhatsApp: col numero del cliente apre direttamente la chat (come i follow-up); senza numero ripiega sul selettore
  const waLink=(phone,link)=>{const d=String(phone||"").replace(/\D/g,"");const p=d.length===10?`39${d}`:d;const text=encodeURIComponent("Ciao! Per il tuo appuntamento compila questo form: "+link);return p?`whatsapp://send?phone=${p}&text=${text}`:`https://wa.me/?text=${text}`;};
  ...
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      <PageHeader title="Agenda" action={<div style={{display:"flex",gap:"6px"}}><Btn v="secondary" s="sm" onClick={()=>{setShowSlot(true);setSlotDone(false);setSlotLink("");setSlotForm({date:today(),serviceType:"",phone:""});}}>Prepara scheda</Btn><Btn s="sm" onClick={()=>setShowNew(true)}>+ Nuovo</Btn></div>} />
...
export default function SlissPlatform() {
  ...
  // Auto-import schede onboarding: una sola volta dopo il caricamento, importa gli slot già compilati dal cliente
  useEffect(()=>{
