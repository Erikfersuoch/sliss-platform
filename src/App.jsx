...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(e,info) { console.error("[Sliss] Crash:", e, info?.componentStack); }
  handleReset() {
    try { Object.keys(localStorage).filter(k=>k.startsWith("sliss")).forEach(k=>localStorage.removeItem(k)); } catch{}
    ...
    if(this.state.hasError) return (
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#F8F9FA",padding:"20px",fontFamily:"sans-serif"}}>
        <div style={{textAlign:"center",maxWidth:"440px"}}>
          ...
          {this.state.msg&&<div style={{background:"#FEF2F2",border:"1px solid #FCA5A5",borderRadius:"8px",padding:"10px 14px",marginBottom:"20px",fontSize:"12px",color:"#DC2626",fontFamily:"monospace",textAlign:"left",wordBreak:"break-all"}}>{this.state.msg}</div>}
  ...
  const renderStep = () => {
    switch(step) {
      ...
      case 2: return <>
        ...
        <Btn onClick={()=>setStep(3)} disabled={!bizType} style={{width:"100%",justifyContent:"center"}}>{"Avanti \u{2192}"}</Btn>
      </>;
      case 3: return <>
        ...
        <Btn onClick={()=>setStep(4)} disabled={!cluster||(isCustomCluster&&!customSector.trim())} style={{width:"100%",justifyContent:"center"}}>{"Avanti \u{2192}"}</Btn>
      </>;
      case 4: return <>
        ...
        <Btn onClick={doComplete} style={{width:"100%",justifyContent:"center"}}>{"Apri Sliss \u{2192}"}</Btn>
      </>;
      default: return null;
    }
  };
...
const getNavMain = (bizType) => {
  const agendaItem = bizType==="prodotti" ? {id:"orders",icon:"\u{1F4E6}",label:"Ordini"} : {id:"appointments",icon:"\u{1F4C5}",label:"Agenda"};
...
const Feedback = () => {
  ...
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px",marginBottom:"20px"}}>{[{label:"Media",value:avgR,color:T.purple},{label:"Totali",value:feedbacks.length,color:T.blue},{label:"Consigliano",value:feedbacks.filter(f=>f.wouldRecommend).length,color:T.green}].map((s,i)=>(<Card key={i} style={{padding:"14px 12px",display:"flex",flexDirection:"column",gap:"4px"}}><span style={{fontSize:"11px",color:T.textD,fontWeight:500}}>{s.label}</span><span style={{fontSize:"24px",fontWeight:700,color:s.color,lineHeight:1}}>{s.value}</span></Card>))}</div>
