...
const Appointments = () => {
  ...
  const handleCreateSlot=()=>{const slotId=uid();const owner=localStorage.getItem('sliss-tester')||'unknown';const biz=data.settings?.businessName||'Sliss';const svcType=slotForm.serviceType||SERVICE_TYPES[0]||"Sessione";const link=`https://sliss-platform.vercel.app/onboarding.html?slot=${slotId}&date=${slotForm.date}&biz=${encodeURIComponent(biz)}&o=${encodeURIComponent(owner)}&svc=${encodeURIComponent(svcType)}`;addRecord("slots",{id:slotId,date:slotForm.date,serviceType:svcType,status:"waiting",created:today(),owner,link});setSlotLink(link);setSlotDone(true);};
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      <PageHeader title="Agenda" action={<div style={{display:"flex",gap:"6px"}}><Btn v="secondary" s="sm" onClick={()=>{setShowSlot(true);setSlotDone(false);setSlotLink("");}}>Prepara scheda</Btn><Btn s="sm" onClick={()=>setShowNew(true)}>+ Nuovo</Btn></div>} />
