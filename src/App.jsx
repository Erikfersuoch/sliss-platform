  ...
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      ...
      <Modal open={showSlot} onClose={()=>{setShowSlot(false);setSlotDone(false);setSlotLink("");}} title="Link per il cliente">
        {slotDone
          ?<div><p style={{fontSize:"13px",color:T.textD,lineHeight:1.6,marginBottom:"14px"}}>Mandalo al cliente su WhatsApp. Appena compila, lo trovi automaticamente tra i clienti.</p><div style={{padding:"12px 14px",background:T.bg3,borderRadius:T.r.m,border:`1px solid ${T.border}`,fontSize:"12px",wordBreak:"break-all",marginBottom:"14px",color:T.textM,lineHeight:1.6}}>{slotLink}</div><Btn style={{width:"100%",justifyContent:"center"}} onClick={()=>window.open("https://wa.me/?text="+encodeURIComponent("Ciao! Per il tuo appuntamento compila questo form: "+slotLink),"_blank")}>{"📲 Invia su WhatsApp"}</Btn></div>
