...
const Appointments = () => {
  ...
  const [showSlot,setShowSlot]=useState(false);const [slotDone,setSlotDone]=useState(false);const [slotForm,setSlotForm]=useState({date:today(),serviceType:""});const [slotLink,setSlotLink]=useState("");
  const cluster=data?.settings?.cluster||"altro_s";
  ...
  const handleCreateSlot=()=>{const slotId=uid();const owner=localStorage.getItem('sliss-tester')||'unknown';const biz=data.settings?.businessName||'Sliss';const svcType=slotForm.serviceType||SERVICE_TYPES[0]||"Sessione";const link=`https://sliss-platform.vercel.app/onboarding.html?slot=${slotId}&date=${slotForm.date}&biz=${encodeURIComponent(biz)}&o=${encodeURIComponent(owner)}`;addRecord("slots",{id:slotId,date:slotForm.date,serviceType:svcType,status:"waiting",created:today(),owner,link});setSlotLink(link);setSlotDone(true);};
  const handleCheckSlot=async slot=>{try{const r=await fetch(`/api/onboarding-check?slot=${slot.id}`);const d=await r.json();if(d.found){update("slots",slot.id,{status:"received",clientName:d.name,clientPhone:d.phone,clientNotes:d.notes||"",receivedAt:d.submitted});}else{alert("Il cliente non ha ancora compilato il form.");}}catch(e){alert("Errore di connessione. Riprova.");}};
  const handleImportSlot=slot=>{const clientId=uid();const apptId=uid();const timings=data.settings?.followUpTimings||{thankyou:0,check:7,review:21,reactivation:60};addRecord("clients",{id:clientId,name:slot.clientName,phone:slot.clientPhone,email:"",channel:"WhatsApp",status:"new",tags:[],notes:slot.clientNotes||"",firstVisit:slot.date,lastVisit:slot.date});addRecord("appointments",{id:apptId,clientId,date:slot.date,serviceType:slot.serviceType,notes:"",followUpTriggered:true,created:today()});buildFollowUps(apptId,clientId,slot.clientName,slot.date,slot.serviceType,timings,data?.templates).forEach(fu=>addRecord("followUps",fu));update("slots",slot.id,{status:"imported"});};
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      <PageHeader title="Agenda" action={<div style={{display:"flex",gap:"6px"}}><Btn v="secondary" s="sm" onClick={()=>{setShowSlot(true);setSlotDone(false);setSlotLink("");}}>Link cliente</Btn><Btn s="sm" onClick={()=>setShowNew(true)}>+ Nuovo</Btn></div>} />
      ...
      {(data?.slots||[]).filter(s=>s.status!=="imported").length>0&&<div style={{marginBottom:"16px"}}><div style={{fontSize:"11px",fontWeight:700,color:T.textD,textTransform:"uppercase",letterSpacing:".06em",marginBottom:"8px"}}>Link inviati</div><div style={{display:"flex",flexDirection:"column",gap:"8px"}}>{(data?.slots||[]).filter(s=>s.status!=="imported").map(slot=>{const rec=slot.status==="received";return (<Card key={slot.id} style={{borderLeft:`4px solid ${rec?T.green:T.amber}`,background:rec?T.greenS:"transparent"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}><div><div style={{fontWeight:600,fontSize:"14px"}}>{slot.serviceType} · {fmtDate(slot.date)}</div><div style={{fontSize:"12px",color:T.textD,marginTop:"2px"}}>{rec?slot.clientName+" · "+slot.clientPhone:"In attesa del cliente"}</div></div><Badge label={rec?"Dati ricevuti":"In attesa"} color={rec?T.green:T.amber} bg={rec?T.greenS:T.amberS} s /></div><div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>{rec?<Btn s="sm" onClick={()=>handleImportSlot(slot)} style={{flex:1,justifyContent:"center"}}>Importa cliente</Btn>:<><Btn v="secondary" s="sm" onClick={()=>handleCheckSlot(slot)}>Controlla</Btn><Btn v="secondary" s="sm" onClick={()=>navigator.clipboard?.writeText(slot.link||"")}>Copia link</Btn></>}<Btn v="danger" s="sm" onClick={()=>update("slots",slot.id,{status:"imported"})}>✕</Btn></div></Card>);})}</div></div>}
      ...
      <Modal open={showSlot} onClose={()=>{setShowSlot(false);setSlotDone(false);setSlotLink("");}} title="Link per il cliente">
        {slotDone
          ?<div><p style={{fontSize:"13px",color:T.textD,lineHeight:1.6,marginBottom:"14px"}}>Copia il link e mandalo al cliente. Appena compila, puoi importare i suoi dati.</p><div style={{padding:"12px 14px",background:T.bg3,borderRadius:T.r.m,border:`1px solid ${T.border}`,fontSize:"12px",wordBreak:"break-all",marginBottom:"14px",color:T.textM,lineHeight:1.6}}>{slotLink}</div><div style={{display:"flex",gap:"8px"}}><Btn style={{flex:1,justifyContent:"center"}} onClick={()=>navigator.clipboard?.writeText(slotLink)}>Copia</Btn><Btn v="secondary" style={{flex:1,justifyContent:"center"}} onClick={()=>window.open("https://wa.me/?text="+encodeURIComponent("Ciao! Per il tuo appuntamento compila questo form: "+slotLink),"_blank")}>{"📲 WhatsApp"}</Btn></div></div>
          :<><FormField label="Data" hint="I follow-up partiranno da questa data"><input type="date" value={slotForm.date} onChange={e=>setSlotForm(p=>({...p,date:e.target.value}))} /></FormField><FormField label="Tipo sessione"><select value={slotForm.serviceType||SERVICE_TYPES[0]} onChange={e=>setSlotForm(p=>({...p,serviceType:e.target.value}))}>{SERVICE_TYPES.map(s=><option key={s}>{s}</option>)}</select></FormField><div style={{display:"flex",gap:"10px",justifyContent:"flex-end"}}><Btn v="secondary" onClick={()=>setShowSlot(false)}>Annulla</Btn><Btn onClick={handleCreateSlot} disabled={!slotForm.date}>Genera link</Btn></div></>
        }
      </Modal>
  ...
  return (
    <div style={{animation:"fadeIn .35s ease"}}>
      ...
      {templates.length===0
        ...
        :<div style={{display:"flex",flexDirection:"column",gap:"10px"}}>{filtered.map((tmpl,i)=>{const ph=PHASES[tmpl.phase]||{icon:"📋",label:tmpl.phase||"",color:T.textD,bg:T.bg3};const isEditing=editingId===tmpl.id;return (<Card key={tmpl.id} style={{animation:`fadeIn .3s ease ${i*.04}s both`,opacity:tmpl.active===false?0.6:1}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"10px"}}><div style={{display:"flex",alignItems:"center",gap:"8px",flex:1,minWidth:0}}><span style={{fontSize:"16px",flexShrink:0}}>{ph.icon}</span>{isEditing?<input value={editName} onChange={e=>setEditName(e.target.value)} style={{fontSize:"14px",fontWeight:600,padding:"6px 10px"}} />:<div style={{minWidth:0}}><div style={{fontWeight:600,fontSize:"14px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{tmpl.name}</div><div style={{fontSize:"11px",color:T.textD,marginTop:"1px"}}>{tmpl.channel}</div></div>}</div><div style={{display:"flex",gap:"6px",alignItems:"center",flexShrink:0,marginLeft:"8px"}}><Badge {...ph} s />{isEditing?<><Btn v="success" s="sm" onClick={()=>saveEdit(tmpl.id)}>{"\u{2713}"}</Btn><Btn v="ghost" s="sm" onClick={()=>setEditingId(null)}>{"\u{2715}"}</Btn></>:<><Btn v="ghost" s="sm" onClick={()=>startEdit(tmpl)}>{"\u{270F}\u{FE0F}"}</Btn><Btn v="danger" s="sm" onClick={()=>{if(window.confirm("Eliminare?"))deleteRecord("templates",tmpl.id);}}>{"\u{1F5D1}\u{FE0F}"}</Btn></>}</div></div>{isEditing?<textarea value={editText} onChange={e=>setEditText(e.target.value)} style={{minHeight:"90px",fontSize:"14px",lineHeight:1.7}} />:<div style={{padding:"12px 14px",background:T.bg3,borderRadius:T.r.m,border:`1px solid ${T.border}`,fontSize:"14px",lineHeight:1.7,color:T.textM,whiteSpace:"pre-wrap"}}>{tmpl.text}</div>}{!isEditing&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"10px"}}><button onClick={()=>update("templates",tmpl.id,{active:tmpl.active===false?true:false})} style={{background:"none",border:`1px solid ${tmpl.active===false?T.border:T.green}`,color:tmpl.active===false?T.textMu:T.green,borderRadius:T.r.m,padding:"7px 14px",fontSize:"13px",cursor:"pointer",fontFamily:"inherit",minHeight:"40px"}}>{tmpl.active===false?"○ Disattivato":"● Attivo"}</button><button onClick={()=>navigator.clipboard.writeText(tmpl.text)} style={{background:"none",border:`1px solid ${T.border}`,color:T.textD,borderRadius:T.r.m,padding:"7px 14px",fontSize:"13px",cursor:"pointer",fontFamily:"inherit",minHeight:"40px"}}>{"\u{1F4CB}"} Copia</button></div>}</Card>);})}</div>
      }
