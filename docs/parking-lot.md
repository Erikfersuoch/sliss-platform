...
- **Link corto invito + invito sotto Clienti (post-gate).** Residui ancora aperti del redesign "Invita cliente" (fatto v6.5 → `archivio/parking-lot-chiusi.md`): (a) **link corto** `sliss…/c/<id>` (slot su Upstash + rotta di redirect) al posto dell'URL lungo nel messaggio; (b) valutare di spostare l'azione *sotto* Clienti togliendola da Agenda (Opzione B).
...
- [28/06/2026] **AI Act UE — orientamento (no panico, è Fase 3).** Domanda di Erik: come gestire l'AI Act per non avere problemi al go-to-market. Sintesi (orientamento, NON consulenza legale):
  **1. Oggi Sliss NON è un "sistema di IA" ai sensi dell'AI Act:** i messaggi sono **template scritti a mano** che l'utente modifica e invia con un tap. Nessun modello genera contenuti o decide in autonomia → l'AI Act, allo stato attuale, ha impatto minimo. (Il nome "Sliss" o l'uso di Claude per *costruirlo* non rilevano: conta cosa fa il software in mano all'utente.)
  **2. Il fronte vero a breve è il GDPR, non l'AI Act:** Sliss tratta nomi/contatti dei clienti finali e invia messaggi → in Fase 3 servono privacy policy, base giuridica/consenso, minimizzazione, retention, e attenzione al backup cloud legato al codice tester (Sliss = responsabile/incaricato del trattamento per conto del professionista titolare → valutare DPA).
  **3. Se in futuro si aggiunge IA generativa** (un modello che *scrive* il follow-up): scatta soprattutto l'obbligo di **trasparenza** (Art. 50 — far capire che il testo è assistito da IA). Resta fascia **"rischio limitato"**: niente high-risk (no biometria, scoring creditizio, selezione personale).
...
- [23/06/2026] **Spec-first per TUTTI i moduli (M2, M5, M6, M9) — come fatto per M3.** Intuizione di Erik: l'approccio usato per M3 Richieste (progettare la spec completa senza costruire nulla, `docs/spec-m3-richieste.md`) va replicato per tutti i moduli futuri. Vantaggi: struttura già convalidata idealmente e visionata, quando arriva il momento di costruire si esegue senza ri-ragionare. **Disciplina:** uno alla volta, al momento naturale (es. quando un tester esprime un dolore legato a quel modulo, o in una sessione CORE tra un gate e l'altro), non tutti insieme — altrimenti è dispersione. **Ordine suggerito:** M2 (Riattivazione, estensione naturale di M1) → M5 (Recensioni, già semi-coperto da M1) → M6 (merge in M3 già deciso) → M9 (Automazione, Fase 3). Ogni spec segue lo stesso formato di `spec-m3-richieste.md`: problema reale → flusso utente → "Fatto quando" → vincoli tecnici → sinergie con gli altri moduli.
