...
1. **Rivedere/approvare l'anteprima `docs/test-m1/home-moduli-anteprima.html`** — Home a moduli (ogni modulo un suo spazio + task, gestire quasi tutto da lì), barra che raggiunge i moduli, "Altro" = gestione Sliss, sinergia visibile col "filo". È un **redesign della scocca → tocca M1 → design-first**, poi costruire. **+ contatori dedicati per modulo dove servono (es. Richieste) [feedback Erik 23/06].** *(Interim già fatto 23/06: Richieste è sulla barra mobile per i prodotti — Home·Richieste·[+]·Ordini·Follow-Up, con badge n. nuove; Clienti spostato in "Altro". Resta la Home-a-moduli completa.)* (Scaling a pacchetti Smart/Pro/Enterprise = Fase 3, in parking-lot.)
